      ...
      gapi.client
        ...
        .then(function() {
          return gapi.client.request({
            path: `https://www.googleapis.com/calendar/v3/calendars/${CALENDAR_ID}/events?maxResults=11&orderBy=updated&timeMin=${moment().tz("Asia/Calcutta|Asia/Kolkata").toISOString()}&timeMax=${moment().tz("Asia/Calcutta|Asia/Kolkata").endOf("day").toISOString()}`
          });
        })
        .then(
          response => {
  ...
  render() {
    ...
    let emptyState = (
      <div className="empty">
        ...
        <h3>
          No Events are scheduled for the day. Create one by clicking the
          button below.
        </h3>
      </div>
    );
